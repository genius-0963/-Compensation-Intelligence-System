...
export default function SavedPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="mb-8">
        <h1 className="text-3xl font-black text-white tracking-tight">Saved Research</h1>
        <p className="text-slate-500 mt-1 font-medium">Access your bookmarked comparisons, searches, and benchmarks.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 animate-fade-up">
        {mockSaved.filter(item => activeTab === 'all' || item.type === activeTab || activeTab === 'searches').map((item, i) => (
          <Card key={i} className="p-6 border-none shadow-sm bg-card hover:shadow-md transition-all group flex items-center justify-between">
            <div className="flex items-center gap-6">
               <div className="h-12 w-12 rounded-2xl bg-[#0B1020] flex items-center justify-center text-gray-400 group-hover:text-blue-600 transition-colors">
                  {tabs.find(t => t.id === item.type)?.icon && React.createElement(tabs.find(t => t.id === item.type)!.icon, { className: 'h-6 w-6' })}
               </div>
               <div>
                  <h3 className="font-bold text-white group-hover:text-blue-600 transition-colors">{item.title}</h3>
            ...
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
               <button className="p-2 text-gray-400 hover:text-blue-600 hover:bg-[#1E3A8A]/50 rounded-lg transition-all"><ExternalLink className="h-4 w-4" /></button>
               <button className="p-2 text-gray-400 hover:text-blue-600 hover:bg-[#1E3A8A]/50 rounded-lg transition-all"><Share2 className="h-4 w-4" /></button>
               ...
               <button className="p-2 text-gray-400 hover:text-white rounded-lg transition-all"><MoreVertical className="h-4 w-4" /></button>
            </div>
          </Card>
        ))}
        ...
        {mockSaved.filter(item => item.type === activeTab).length === 0 && activeTab !== 'searches' && (
          <div className="py-20 text-center space-y-4">
             <div className="h-16 w-16 bg-[#0B1020] rounded-[24px] flex items-center justify-center mx-auto text-gray-300">
                <Bookmark className="h-8 w-8" />
             </div>
             <div>
                <h3 className="font-black text-white">No saved {activeTab} yet</h3>
